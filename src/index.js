...
MT.tools.format = function (str, arg, key) {
  for (var i = 0; i < arg.length; i++) {
    ...
    if (arg[i]) {
      if (arg[i] === "Y" || arg[i] === "M" || arg[i] === "D")
        arg[i] = MT.tools.pad(arg[i]);
  ...
  if (elems.length > 0) {
    ...
    if (newElems.length > 0) {
      for (var i = 0; i < newElems.length; i++) {
        ...
        MT.names[timeData.name]
          ? MT.names[timeData.name].push(nameData)
          : [nameData];
      }
    }
  }
...
MT.build = function (selector) {
  ...
  for (var i = 0; i < MT.timebase[groupIndex].length; i++) {
    ...
    MT.jobs[groupIndex][i] = setInterval(
      (function (i) {
        return function () {
          MT.working(groupIndex, i);
        };
